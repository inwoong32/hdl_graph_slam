...
public:
  ...
  virtual void onInit() {
    ...
    gps_frame_id = private_nh.param<std::string>("gps_frame_id", "/gnss");
    ...
    // imu_pub = mt_nh.advertise<sensor_msgs::Imu>("/imu_data", 16);
    // publishers
...
private:
  ...
  void cloud_callback(const nav_msgs::OdometryConstPtr& odom_msg, const sensor_msgs::PointCloud2::ConstPtr& cloud_msg) {
    ...
    if(!keyframe_updater->update(odom)) { // update() --> 최초로 호출되면 true return  // 변환값이 너무 작으면(프레임간 간격이 너무 작으면) false
    ...
    double accum_d = keyframe_updater->get_accum_distance(); // 총 움직인 거리 
   ...
   */
  bool flush_keyframe_queue() { //keyframe_queue에 있는 내용으로  graph_slam 객체 업데이트
    std::lock_guard<std::mutex> lock(keyframe_queue_mutex);
    ...
    int num_processed = 0;
    for(int i = 0; i < std::min<int>(keyframe_queue.size(), max_keyframes_per_update); i++) { //keyframe_queue 사이즈 혹은 업데이트 변수 만큼 반복하여 process
      num_processed = i;
  ...
  void imu_callback(const sensor_msgs::ImuPtr& imu_msg) {
      ...
      // sensor_msgs::ImuPtr imu_data = imu_msg;

      // imu_data->header.frame_id = "my_imu";
  ...
  bool flush_imu_queue() {
    ...
    for(auto& keyframe : keyframes) {
      ...
      if(enable_imu_acceleration) {
        ...
        // IW : change Eigen::Vector3d::UnitZ() scale
        g2o::OptimizableGraph::Edge* edge = graph_slam->add_se3_prior_vec_edge(keyframe->node, Eigen::Vector3d::UnitZ(), *keyframe->acceleration, info);
   ...
   * @param event
   */
   // 일정 시기 혹은 graph update가 true가 되면 keyframe 정보로 전체 맵 생성 후 publish
  void map_points_publish_timer_callback(const ros::WallTimerEvent& event) { 
   
    if(!map_points_pub.getNumSubscribers() || !graph_updated) { // map_point sub이 없거나 graph_update가 false면 함수 종료
      return;
    }
    ...
    auto cloud = map_cloud_generator->generate(snapshot, map_cloud_resolution); // 현재까지 있는 pointcloud 데이터를 이용하여 map 생성 
    if(!cloud) {
      return;
    }
  ...
  void optimization_timer_callback(const ros::WallTimerEvent& event) {
    ...
    bool keyframe_updated = flush_keyframe_queue(); //keyframe_pueue가 비어있으면 false
    ...
    // 각 queue 정보를 graph_slam 에 등록
    if(!keyframe_updated & !flush_floor_queue() & !flush_gps_queue() & !flush_imu_queue()) { //각 queue가 잘 작동했고 keyframe_update가 ture 이면 계속 진행
      return;
    }
  ...
  bool load_service(hdl_graph_slam::LoadGraphRequest& req, hdl_graph_slam::LoadGraphResponse& res) {
    ...
    keyframes_snapshot_mutex.lock();
    keyframes_snapshot.swap(snapshot); //메모리 공간까지 삭제
    keyframes_snapshot_mutex.unlock();
    graph_updated = true;
