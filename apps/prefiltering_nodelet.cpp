...
private:
  void initialize_params() {
    ...
    if(outlier_removal_method == "STATISTICAL") {
      ...
      sor->setMeanK(mean_k); //고려할 이웃 점의 수
      sor->setStddevMulThresh(stddev_mul_thresh); //Outlier로 처리할 거리 정보 (임계치 값으로 값이 작을수록 aggressive 하게 적용)
      ...
      rad->setRadiusSearch(radius); // 분석시 고려할 반경 범위
      rad->setMinNeighborsInRadius(min_neighbors); // Outlier로 처리할 거리정보(탐색 범위내에 필요한 최소 포인트 수)
